...
pub fn ddmin<T, F>(items: Vec<T>, target: TestOutcome, mut test: F) -> Result<MinimizeResult<T>, MinimizeError>
where
    T: Clone,
    F: FnMut(&[T]) -> TestOutcome,
{
    ...
    if items.len() < 2 {
        return Ok(MinimizeResult { items, tests_executed: tests });
    }
    ...
    loop {
        ...
        // First ask whether a whole chunk can be removed while preserving the target.
        ...
        // Then ask whether one chunk alone preserves the target.
        for (start, end) in &ranges {
    ...
    Ok(MinimizeResult { items: current, tests_executed: tests })
}
...
mod tests {
    ...
    fn minimizes_failure_to_single_trigger() {
        let result = ddmin(vec![1, 2, 3, 4, 5, 6], TestOutcome::Fail, |items| {
            if items.contains(&3) { TestOutcome::Fail } else { TestOutcome::Pass }
        }).unwrap();
    ...
    fn minimizes_success_to_fields_the_consumer_needs() {
        let result = minimize_success(vec!["id", "name", "email", "avatar", "metadata"], |items| {
            if items.contains(&"id") && items.contains(&"email") { TestOutcome::Pass } else { TestOutcome::Fail }
        }).unwrap();
