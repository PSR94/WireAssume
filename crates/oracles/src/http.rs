...
impl HttpOracle {
    pub fn new(spec: HttpOracleSpec) -> Result<Self, OracleError> {
        ...
        if spec.success_statuses.is_empty() {
            return Err(OracleError::Invalid(
                "success_statuses must not be empty".into(),
            ));
        }
...
impl Oracle for HttpOracle {
    ...
    async fn evaluate(&self) -> Result<OracleResult, OracleError> {
        ...
        let body = response.text().await.map_err(|error| {
            OracleError::Execution(format!("failed to read HTTP oracle body: {error}"))
        })?;
        ...
        Ok(OracleResult {
            ...
            summary: if failures.is_empty() {
                format!("HTTP oracle passed with status {status_code}")
            } else {
                failures.join("; ")
            },
            stdout: body,
...
fn default_statuses() -> BTreeSet<u16> {
    (200..300).collect()
}
