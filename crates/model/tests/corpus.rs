use serde_json::json;
use wireassume_model::{Body, CorpusStore, Header, Interaction, InteractionMetadata, RedactionPolicy, RequestRecord, ResponseRecord, REDACTED};
...
fn sample() -> Interaction {
    Interaction {
        request: RequestRecord {
            ...
            headers: vec![Header { name: "Authorization".into(), value: "Bearer top-secret".into() }],
            body: Body::Empty,
        },
...
fn corpus_ids_are_deterministic_and_secrets_are_not_persisted() {
    ...
    let first = store.persist(&sample(), &RedactionPolicy::default()).unwrap();
    let second = store.persist(&sample(), &RedactionPolicy::default()).unwrap();
