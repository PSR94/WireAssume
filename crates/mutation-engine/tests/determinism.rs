...
use wireassume_mutation_engine::{ArrayMutator, JsonMutator, MutationKind, MutationPlanner, PlannerConfig};
...
fn same_seed_produces_same_mutation_ids_and_payloads() {
    ...
    let config = PlannerConfig { budget: 200, seed: 42, ..PlannerConfig::default() };
...
fn removal_nullability_type_and_order_mutations_are_real_payload_changes() {
    ...
    let config = PlannerConfig { budget: 200, seed: 7, ..PlannerConfig::default() };
    let plan = planner().plan(&baseline, &config, &HashSet::new());

    for kind in [MutationKind::RemoveField, MutationKind::NullField, MutationKind::WrongPrimitiveType, MutationKind::ReverseArray] {
        let mutation = plan.selected.iter().find(|mutation| mutation.kind == kind).expect("required mutation family");
...
fn planner_budget_and_seen_ids_prevent_reexecution() {
    ...
    let config = PlannerConfig { budget: 3, ..PlannerConfig::default() };
    let first = planner().plan(&baseline, &config, &HashSet::new());
    ...
    assert!(second.selected.iter().all(|m| !first.selected.iter().any(|old| old.id == m.id)));
}
