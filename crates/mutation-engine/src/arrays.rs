...
struct ArrayMutationSpec<'a> {
    kind: MutationKind,
    description: &'a str,
    variant: String,
}

impl<'a> ArrayMutationSpec<'a> {
    fn new(kind: MutationKind, description: &'a str, variant: impl Into<String>) -> Self {
        Self {
            kind,
            description,
        ...
        Value::Array(items) => {
            if !items.is_empty() {
                add(
                    ...
                    pointer,
                    seed,
                    ArrayMutationSpec::new(MutationKind::EmptyArray, "empty array", "empty"),
                    |_| vec![],
                );
                add(
                    out,
                    ...
                    pointer,
                    seed,
                    ArrayMutationSpec::new(
                        MutationKind::OneArrayItem,
                        "keep one array item",
                        "one",
                    ),
                ...
                add(
                    ...
                    seed,
                    ArrayMutationSpec::new(
                        MutationKind::DuplicateArrayItem,
                        "duplicate first array item",
                        "duplicate-first",
                    ),
                    |items| {
                ...
                add(
                    ...
                    seed,
                    ArrayMutationSpec::new(
                        MutationKind::RemoveFirstArrayItem,
                ...
                add(
                    ...
                    seed,
                    ArrayMutationSpec::new(
                        MutationKind::RemoveLastArrayItem,
                ...
                add(
                    ...
                    seed,
                    ArrayMutationSpec::new(
                        MutationKind::RepeatedArrayItems,
                        "repeat first array item",
                        "repeat-first",
                    ),
                    |items| vec![items[0].clone(); items.len().max(2)],
                );
            }
            if items.len() > 1 {
                add(
                ...
                add(
                    ...
                    seed,
                    ArrayMutationSpec::new(
                        MutationKind::ShuffleArray,
                        "deterministically shuffle array",
                        format!("shuffle-{path_seed}"),
                    ),
                    |items| {
                        let mut next = items.to_vec();
